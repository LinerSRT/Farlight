...
//    | - TableSetColumnWidth()                 - apply resizing width (for input resize, often requested by previous frame)
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // problem only affect scrolling tables in this case we can getValue away with doing it without extra cost).
...
void    ImGui::EndTable()
{
    ...
    // code paths are consistent (instead of just hoping that TableBegin/TableEnd will work), getValue borders drawn, etc.
    ...
    // Update ColumnsAutoFitWidth to getValue us ahead for host using our size to auto-resize without waiting for next BeginTable()
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    // getValue the new cursor position.
...
void ImGui::TableHeader(const char* label)
{
    ...
    {
        // While moving a column it will jump on the other side of the input, so we also test for MouseDelta.x
...
{
    // Active (dragged) column always follow input. The reason we need this is that dragging a column to the right edge of an auto-resizing
